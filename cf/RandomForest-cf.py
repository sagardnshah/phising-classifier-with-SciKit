...
x_train=x_train1.drop(columns=['domain'])
x_test=x_test1.drop(columns=['domain'])
...
classifier = RandomForestClassifier(n_estimators = 500, criterion = "gini", max_features = 'sqrt',  random_state = 0)
...
phish.to_csv(r"C:\Users\sshah65\Google Drive\UNCC\Security Analytics\Project 3 - Phising Classifier with SciKit\code\cf\cf-phish.csv", index = None, header=True)
nonphish.to_csv (r"C:\Users\sshah65\Google Drive\UNCC\Security Analytics\Project 3 - Phising Classifier with SciKit\code\cf\cf-nonphish.csv", index = None, header=True)
